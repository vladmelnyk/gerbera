...
public class SigPreimageProducer {

    public byte[] produce(List<Input> inputs, List<Output> outputs, int singingInputIndex) {
        if (inputs.get(singingInputIndex).isSegWit()) {
            return produceSegwit(inputs, outputs, singingInputIndex);
        } else {
            return produceRegular(inputs, outputs, singingInputIndex);
        }
    }

    private byte[] produceRegular(List<Input> inputs, List<Output> outputs, int singingInputIndex) {
    ...
    private byte[] produceSegwit(List<Input> inputs, List<Output> outputs, int signedIndex) {
        ByteBuffer result = new ByteBuffer();
